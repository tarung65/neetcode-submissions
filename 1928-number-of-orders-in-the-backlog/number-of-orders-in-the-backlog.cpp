class Solution {
    using ORDER = std::tuple<int,int,int>;
            ...
            if(o[2] == 0){ //buy order
                while(!sellOrder.empty() && std::get<0>(sellOrder.top()) <= o[0] && o[1] > 0){
                    ...
                    if(amount <= o[1]){ //sell order is smaller than buy order
                        ...
                        amount -= o[1];
                        o[1] = 0;
                        sellOrder.push({price, amount, orderType});
                        break;
                    }
                }
                if(o[1] > 0) buyorder.push({o[0], o[1], o[2]});
            } else { //sell order
                while(!buyorder.empty() && std::get<0>(buyorder.top()) >= o[0] && o[1] > 0){
                    auto [price, amount, orderType] = buyorder.top();
                    buyorder.pop();
                    if(amount <= o[1]){ //buy order is smaller than sell order
                        ...
                        amount -= o[1];
                        o[1] = 0;
                        buyorder.push({price, amount, orderType});
                        break;
                    }
                }
                if(o[1] > 0) sellOrder.push({o[0], o[1], o[2]});
            }
        }
        long long backlog = 0;
